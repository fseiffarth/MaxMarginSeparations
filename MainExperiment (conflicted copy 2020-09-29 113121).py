    ...
    def evaluation_to_database(self, database, data_id):
        ...
        for i in range(len(self.accuracy)):
            database.experiment_to_database(table_name, columns, [
                [time.time(), self.algo[i], self.accuracy[i], self.w_coverage[i], self.coverage[i], self.w_coverage[i],
...
def run_single_experiment(database, openmlData, data_id, algos, train_size, number, print_time = False, plot=False):
                    ...
                    if algo == "greedy":
                        ...
                        if algo == "OvOstd":
                            ...
                            if print_time:
                                print("One vs. one {}".format(new_time - start))
                            eval.set_values(openmlData, algo, training_sets, prediction, new_time - start)
        print("Num: {}/{}".format(num, number) + " Estimated Duration: " + str((number-num) * (time.time()-start_time)/num) + "s")
    eval.evaluation_to_database(database, data_id)

def run_synthetic(dim=2, number, plot=False)
    run_start = time.time()
    print(data_id, number)
    for i in range(number):
        openmlData = OpenMLData(db_id=dim-1, multilabel=True)
        train_sizes = openmlData.get_train_sizes()
    Parallel(n_jobs=5)(delayed(run_single_experiment)(database, openmlData, data_id, algos, x, 1, plot) for x in train_sizes)
    print("Run Duration: ", time.time() - run_start, " s")

def run(database, algos, data_id, number, plot=False):
    run_start = time.time()
    print(data_id, number)
    openmlData = OpenMLData(db_id=data_id, multilabel=True)
    train_sizes = openmlData.get_train_sizes()
    Parallel(n_jobs=5)(delayed(run_single_experiment)(database, openmlData, data_id, algos, x, number, plot) for x in train_sizes)
    print("Run Duration: ", time.time() - run_start, " s")
